...
//	int j;
	...
	for(i = 1023; i >= 0; i--){
		...
		if((pt_addr =pgd_ptr[i].pgd) ){	//print the pgd entries which are not NULL
			
			pt_addr = ((pt_addr >> 12) << 12); //masking last 3 nibbles
			
			printk("PGD[%04d] : %08lx \n", i, pt_addr);//printing pgd entries (i.e base physical address of page tables)
			
			pg_num = pt_addr >> 12; //calculating pg_num (i.e index to mem_map array)
			
			//coverting physical to virtual memory
		
			if(pt_addr <= 0x38000000){ //if phy_addr is in LOW MEM
				v_addr = (void *)(pt_addr + 0xc0000000);
