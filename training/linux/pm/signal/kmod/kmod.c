...
//	struct task_struct *my_task = current;
	...
	printk(KERN_INFO "Open My_ioctl successful \n");
	printk(KERN_INFO "my_pid in ioctl using structure : %d \n", my_task->pid);
	printk(KERN_INFO "string in comm using struct is : %s \n", my_task->comm);
	printk(KERN_INFO "PID in Ioctl is %u \n", a);
	printk(KERN_INFO "name of my kernel module : %s \n",my_module->name);
