...
long myIoctl (struct file * fi, unsigned int a, unsigned long b)
{
	...
	printk(KERN_INFO "\nfile_struct address : %p \n", my_task->files);          
    printk(KERN_INFO "fdtable address : %p \n", my_task->files->fdt);           
    printk(KERN_INFO "session table address : %p \n", my_task->files->fdt->fd);
	if(my_task->files->fdt->fd[b]){

		printk(KERN_INFO "session address : %p \n", *((my_task->files->fdt->fd) + b));
		printk(KERN_INFO "f_count : %d \n", my_task->files->fdt->fd[b] -> f_count.counter);
	}else
		printk(KERN_INFO "Session is not present \n");
