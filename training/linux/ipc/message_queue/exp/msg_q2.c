...
//		printf("Send the data into queue : \n");
		...
		if(-1 == (mqdes = mq_open("/msg_queue", O_CREAT | O_WRONLY, S_IWUSR))){	//creating (opening) queue
			errno_abort("queue is not opened for sending...! \n");
		}	
		...
		if(-1 == (mqdes = mq_open("/msg_queue", O_RDONLY, S_IRUSR))){	//creating (opening) queue
			errno_abort("queue is not opened for receiving...! \n");
		}	
