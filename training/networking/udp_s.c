...
int main(void)
{
	...
	my_addr.sin_port = 0;//htons(1234);
	my_addr.sin_addr.s_addr = 0;//inet_addr("172.16.5.56"); //htonl();
	...
	if (-1 == getsockname(sockd, (struct sockaddr *)my_addr, &len)){            
        errno_abort("getting attributes failed....! \n");                       
    }
